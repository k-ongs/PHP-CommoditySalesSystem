...
<body>
			...
			<td class="padding-a" style="border:1px solid #000">
				<?PHP
					if($message_count == 0)
					{
						...
						$offset = ($page-1) * 8;
						//分页查询 每页显示8条记录
						$re = select($mysql,'tb_seckill', 'a join tb_commodity b on a.cId = b.cId where a.startDate <=now() and a.endDate >= now() limit '. $offset .',8','a.nums,a.num,a.recommends,b.cId,b.name,b.price,b.figure');
						foreach($re as $val)
						{
				?>
	...
	<?PHP if($message_count > 0) echo makePage($page_count, $page, 'seckill.php'); ?>
	<br>
	<?PHP echoFooter(); ?>
</body>
